...
const renderChart2 = async function(data){
  ...
  .attr('x', 3).attr('y', d => yScale.bandwidth()/2 + yScale(yValue2(d))).attr('fill', 'black');
  
  var yearText = mainGroup.append('text')
    .attr('class', 'year')
  ...
  .attr('x', d => 3+xScale(xValue2(d))).attr('y', d => yScale.bandwidth()/2 + yScale(yValue2(d)))
...
d3.csv("static/csv/undesa_pd_2020_numbers.csv").then(async data => {
    ...
    // xAxisGroup.append('text').attr('class', 'axistitle2').text('Refugee Stock')
    // .attr('x', innerWidth2/2).attr('y', 60);
    ...
    for (let i = 0; i < years.length; i++) {
        const year = +years[i]; 
        await renderChart2(separatedData.filter(d => d.Year === year));
    }
